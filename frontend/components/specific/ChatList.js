...
const ChatList = () => {
  ...
  const groupselected = useSelector(state => state.selectedUser.isgroupselected);
    ...
    <div className={styles.chatContainer}>
      {groupdata.map((group, index) => {
        ...
        return (
          <div
            key={index}
            className={`${styles.chatItem} ${selectedGoogleID === group.groupID ? styles.isselected : ''}`}
            onClick={() => handleGroupClick(group.groupID)}>
            <span className={styles.avatar}>
              <Image src={group.groupImage} height={40} width={40} alt={group.groupName || 'User avatar'} />
            </span>
            <span className={styles.chatInfo}>
              <div className={styles.friendName}>{group.groupName}</div>
              <div className={styles.newMessages}>
                {unreadCount > 0 ? `${unreadCount} new message(s)` : 'No new messages'}
              </div>
            </span>
      ...
      {totalpeople.map((friend, index) => {
        ...
        return (
          <div
            key={index}
            className={`${styles.chatItem} ${selectedGoogleID === friend.GoogleID ? styles.isselected : ''}`}
            onClick={() => handleClick(friend.GoogleID)}>
            <span className={styles.avatar}>
              <Image src={friend.image} height={40} width={40} alt={friend.name || 'User avatar'} />
            </span>
            <span className={styles.chatInfo}>
