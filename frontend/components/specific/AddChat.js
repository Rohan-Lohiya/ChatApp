...
const AddChat = () => {
  ...
  const handleclick = async e => {
    if (!available) {
      ...
      try {
        const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/check-user`, {
          googleID: email,
        });
  ...
  return (
    <div className={styles.addchatcont}>
      ...
      <div className={styles.inputcont}>
        <input onChange={handlechange} value={email} type="text" placeholder="Enter GmailID of your friend" />
        <button onClick={handlerefresh} className={styles.refreshbtn}>
          Refresh
        </button>
      </div>
      <button onClick={handleclick} disabled={disablebtn} className={styles.searchButton}>{`${
        available ? 'Add' : 'Search'
      }`}</button>
      {message && <div>{available ? `${searchpeson} found` : `${searchpeson} not found`}</div>}
    </div>
  );
};
